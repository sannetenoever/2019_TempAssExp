...
expname = 'AVTIME';
tpv = [50 175];
load('D:\Experiments\StartTimeExp\AA_EEG\04_MidData\AVTIME\S02_01_blinktrl', 'alltr');
...
for pp = 1:length(ppnames)
    ...
    for pr = 1:length(pra)
        for asna = 1:2
            for resp = 1:2
                trl = find(FF.trialinfo(:,4) == asna-1  & FF.trialinfo(:,7)+1 == resp & FF.trialinfo(:,9) >= 0 & alltr{pp}');
                PPi = [];
                for trc = 1:length(trl)
                    in = find(ismember(FF.trialinfo(:,11)-FF.trialinfo(trl(trc),11),pra{pr}) & FF.trialinfo(:,9) >= 0 & alltr{pp}');
                    if length(in) >= 1
...
save(['D:\Experiments\StartTimeExp\AA_EEG\04_MidData\AVTIME\SaveD\S05_00_NB_PowAccInterStat' infor.pr{pr}],'st');

%% plot the interaction:
pr=1;
load(['D:\Experiments\StartTimeExp\AA_EEG\04_MidData\AVTIME\SaveD\S05_00_NB_PowAccInterStat' infor.pr{pr}],'st');
...
exportfig(gcf, ['D:\Experiments\StartTimeExp\AA_EEG\05_Figures\AVTIME\S05_00_NB_PowAccInterTopo_' infor.pr{pr}], 'Color', 'rgb');
print(['D:\Experiments\StartTimeExp\AA_EEG\05_Figures\AVTIME\S05_00_NB_PowAccInterTopo_' infor.pr{pr}],'-dpng');
...
save(['D:\Experiments\StartTimeExp\AA_EEG\04_MidData\AVTIME\SaveD\S05_00_NB_PowAccSimpleStat' infor.pr{pr}],'st30','st70');

%% look at simple effects
pr=1;
load(['D:\Experiments\StartTimeExp\AA_EEG\04_MidData\AVTIME\SaveD\S05_00_NB_PowAccInterStat' infor.pr{pr}],'st70', 'st30');
load('D:\Other\Programs\Matlab\fieldtrip-20120904\fieldtrip-20161231\template\layout\easycapM1');
cfg = [];
cfg.layout = lay;
cfg.parameter = 'stat';
...
set(gcf, 'position',[500 500 375 270]);
exportfig(gcf, ['D:\Experiments\StartTimeExp\AA_EEG\05_Figures\AVTIME\S05_00_NB_SimplePowAcc_' infor.pr{pr}], 'Color', 'rgb');
print(['D:\Experiments\StartTimeExp\AA_EEG\05_Figures\AVTIME\S05_00_NB_SimplePowAcc_' infor.pr{pr}],'-dpng')
...
for pp = 1:length(ppnames)
    ...
    for trc = 1:length(FF.trialinfo)
        in = find(ismember(FF.trialinfo(:,11)-FF.trialinfo(trc,11),pra{pr}) & FF.trialinfo(:,9) >= 0 & alltr{pp}');        
...
bigmat = bigmat';
save([saveloc 'S05_00_NB_BIGMAT'], 'bigmat');
save([saveloc 'S05_00_NB_BIGMAT_varnames'], 'varnames');
...
exportfig(gcf, ['D:\Experiments\StartTimeExp\AA_EEG\05_Figures\AVTIME\S05_00_NB_BehLowPower_' infor.pr{pr}], 'Color', 'rgb');
print(['D:\Experiments\StartTimeExp\AA_EEG\05_Figures\AVTIME\S05_00_NB_BehLowPower_' infor.pr{pr}],'-dpng')
