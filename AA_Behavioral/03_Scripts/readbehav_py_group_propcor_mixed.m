%% read the files
clear 
saveloc = 'D:\Experiments\StartTimeExp\AA_Behavioral\04_MidData\';
...
d = mean(v,2);
ol = [ol1; find(d > median(d) + 1.5*iqr(d)); find(d < median(d) - 1.5*iqr(d))];
...
%% prepare for R to do GLM binomial
bigmat = [];
bigmat2 = [];
varnames = {'accuracy';'sound';'ass';'block';'pp';'curval'; 'dev'};
...
bigmat = bigmat';
bigmat2 = bigmat2';
save([saveloc 'BIGMAT'], 'bigmat');
save([saveloc 'BIGMAT_varnames'], 'varnames');

%% RM anova
restoredefaultpath();
addpath('D:\Other\Programs\Matlab\extrausefullScripts\');
ft.propcor = asin(allv.proplp(ppto,:,:,:));
